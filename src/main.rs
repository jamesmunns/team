...
struct TeamNode {
    team: Team,
    children: HashMap<String, TeamNode>,
}

impl TeamNode {
    fn home(&mut self, other: &Team) -> bool {
        for (_name, ch) in self.children.iter_mut() {
            if ch.home(other) {
                return true;
            }
        }
        if let Some(parent) = other.subteam_of() {
            if parent == self.team.name() {
                let old = self.children.insert(other.name().into(), TeamNode {
                    team: other.clone(),
    ...
    fn dump(&self, spaces: usize) {
        for _ in 0..spaces {
            print!(" ");
        }
        println!("* {} ({:?})", self.team.name(), self.team.kind());
        let mut all_ch = self.children.values().collect::<Vec<_>>();
        all_ch.sort_by_key(|ch| (ch.team.kind(), ch.team.name()));
        for ch in all_ch {
            ch.dump(spaces + 2);
        }
    }
...
fn run() -> Result<(), Error> {
    ...
    let mut teams: HashMap<String, TeamNode> = HashMap::new();

    // Get all the teams
    let mut all_teams = data.teams().collect::<Vec<_>>();

    println!("Pass 0: {} remain (all teams)", all_teams.len());
    // Extract all of the root teams
    all_teams.retain(|t| {
        if let Some(true) = t.top_level() {
            let old = teams.insert(
                t.name().into(),
                TeamNode {
                    team: (*t).clone(),
                    children: HashMap::new(),
    ...
    println!("Pass 1: {} remain (top level teams)", all_teams.len());

    let mut last = all_teams.len();
    let mut i = 1;
    loop {
        if last == 0 {
            println!("All teams sorted!");
            break;
        }
        all_teams.retain(|t| {
            for (_name, tn) in teams.iter_mut() {
                if tn.home(t) {
                    return false;
                }
            }
        ...
        let new_remain = all_teams.len();
        i += 1;
        println!("Pass {}: {} remain", i, new_remain);
        if last == new_remain {
    ...
    all_teams.sort_by_key(|t| (t.kind(), t.name()));

    let mut top_teams = teams.values().collect::<Vec<_>>();
    top_teams.sort_by_key(|t| (t.team.kind(), t.team.name()));

    for t in top_teams {
        t.dump(0);
    }

    println!("* Orphan Teams (without parents)");
    for t in all_teams {
